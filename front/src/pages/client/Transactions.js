...
const Transactions = () => {
  ...
  const getTransactionTypeLabel = (transaction) => {
    if (transaction.proposalId) {
      return 'Paiement avocat';
    } else if (transaction.description?.includes('Abonnement')) {
      return 'Abonnement';
    } else if (transaction.description?.includes('crédits')) {
  ...
  const filterTransactionsByType = (transaction) => {
    switch (filter) {
      ...
      case 'credits':
        return transaction.description?.includes('crédits');
      default:
        return true;
    }
  };
  ...
  const filterTransactionsBySearch = (transaction) => {
    ...
    return (
      ...
      (transaction.lawyerName && transaction.lawyerName.toLowerCase().includes(searchLower)) ||
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {transaction.commission > 0 && (
                        <div className="text-xs text-gray-500">
                          dont commission: {formatPrice(transaction.commission)}
                        </div>
                      )}
                    </td>
